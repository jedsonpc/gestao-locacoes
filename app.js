...
const appVersion = "local-1.8.4";
const appDeployedAt = "2026-06-26T12:15:02-03:00";
...
function isPropertyExcludedFromOccupancy(property) {
  const normalizedType = String(property?.type || "")
    ...
    .toLowerCase()
    .trim();
  return normalizedType === "apartamento" || normalizedType === "imovel residencial";
}
...
function renderErpPropertyProfitability(period, payments, expenses) {
  const rows = state.properties
    .map((property) => {
      ...
      const investment = Number(property.investmentValue || 0);
      return { property, revenue, expenses: expenseTotal, result, margin, roi, investment };
    })
      ...
      return `
        <td>
          <div class="property-profitability-title">
            ...
            <div>
              <strong>${escapeHtml(row.property.description)}</strong>
              <span>${row.investment ? `Investimento: ${formatMoney(row.investment)}` : "Investimento nao informado"}</span>
            </div>
          </div>
        </td>
...
function getErpExportData() {
  ...
  return {
    ...
    profitability: {
      headers: ["Imovel", "Investimento", "Receita", "Despesas", "Resultado", "Margem %", "ROI anualizado %", "ROI total %", "Leitura"],
      rows: propRows,
    },
  };
}
